...
Use the `subagent` tool with `subagent_type: review` and this exact prompt:
...
- **If the `subagent` tool fails or the review returns no verdict** (timeout, crash, tool unavailable): report the error to the user and STOP.
