...
- **No leading `~` in MCP server paths** in `opencode.jsonc` - opencode resolves `mcp.cwd` and `mcp.environment.*` via Node's `path.resolve`, which does NOT expand tilde. A value like `~/.config/opencode/mcp-servers/foo` becomes `<baseDir>/~/.config/...`, the spawned stdio server ENOENTs, and the MCP shows `Failed !` with no other useful error. Always use absolute paths (`/Users/...`) for `cwd`, `environment.PATH`, `environment.DOCS_PATH`, and any other path-like environment variable. Diagnostic: if a local MCP shows `Failed !` while `polars-mcp` (which has no `cwd`/`environment`) stays `Connected ✓`, the cause is almost always a leading `~` somewhere in the failed server's config.
